...
export class UserService {
  ...
  async findById(id: number): Promise<Record<string, any>> {
    ...
    const user = plainToClass(UserEntity, redis_user, { enableImplicitConversion: true })
    if (!user?.id) {
      ...
      await this.redisService
        .getClient('admin')
        .hmset(`${RedisKeyPrefix.USER_INFO}${id}`, instanceToPlain(user))
    }
  ...
  async find(id: number): Promise<Record<string, any>> {
    ...
    const user = await this.userRepo
      ...
      .addSelect(['role.name'])
      .leftJoin('user.userPosts', 'userPosts')
      .addSelect(['userPosts.id'])
      .leftJoin('userPosts.post', 'post')
      .addSelect(['post.post_name', 'post.post_code'])
      .getOne()
    return instanceToPlain(user)
  }
  ...
  async updateById(dto: UpdateUserDto): Promise<Record<string, any>> {
    ...
    if (dto.password) {
      if (dto.password !== dto.confirmPassword)
        throw new HttpException('账号或密码错误', HttpStatus.NOT_ACCEPTABLE)
    ...
    await this.redisService
      .getClient('admin')
      .hmset(`${RedisKeyPrefix.USER_INFO}${dto.id}`, instanceToPlain(dto))
